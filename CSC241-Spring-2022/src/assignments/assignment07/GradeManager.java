...
public class GradeManager {
	...
	public static void main(String[] args) throws Exception {
		...
		while (!quit) {
			...
			System.out.print("Select menu [find | add | remove | edit | print | quit]? ");
			...
			if (actionString.equalsIgnoreCase("find")) {
				...
				editHandler();
			} else if (actionString.equalsIgnoreCase("print")) {
				printHandler();
	...
	private static void removeHandler() {
		...
		for (int i = 0; i < section.students.length; i++) {
			...
			} else if (searchType.equalsIgnoreCase("grade")) {
				if (searchKey.equalsIgnoreCase("" + student.getGrade())) {
					section.removeStudent(student);
				}
			}
		}
