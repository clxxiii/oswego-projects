package edu.oswego.mineserver.object;
...
public class Grid {
    ...
    public Grid(JSONObject grid) {
        ...
        flags = grid.getInt("flags");
        bombs = grid.getInt("bombs");

        revealLock = new ReentrantLock();
        cells = new Cell[height][width];
        JSONArray jsonGrid = grid.getJSONArray("grid");
        for (int y = 0; y < height; y++) {
            JSONArray row = jsonGrid.getJSONArray(y);
            for (int x = 0; x < width; x++) {
                JSONObject cell = row.getJSONObject(x);
                boolean bomb = false;
                int value = 0;
                boolean revealed = cell.getBoolean("revealed");
                boolean flagged = cell.getBoolean("flagged");
                if (revealed) {
                   bomb = cell.getBoolean("bomb");
                   if (!bomb) {
                       value = cell.getInt("value");
                   }
                }
                cells[y][x] = new Cell(bomb, value);
                if (revealed) cells[y][x].reveal();
                if (flagged) cells[y][x].flagged = true;
            }
        }
    }
